...
- The worker refuses a job it cannot run before it joins (`plan`): pool mismatch, non-cascaded mode, e2ee (it holds no key yet), an unregistered provider, a language a provider does not declare, a turn strategy it cannot run, an unknown persona. Providers are constructed and TTS prewarmed before `ctx.connect`.
